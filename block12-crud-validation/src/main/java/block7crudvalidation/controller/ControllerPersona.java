...
////        return restTemplate.getForObject(profesorServiceUrl, ProfesorOutputDto.class);
    ...
    //Bloque12
    @GetMapping("/buscar")
    public ResponseEntity<Page<PersonOutputDto>> buscarPersonas(
            @RequestParam(required = false) String usuario,
            @RequestParam(required = false) String name,
            @RequestParam(required = false) String surname,
            @RequestParam(required = false) Date created_date,
            @RequestParam(required = false) Date termination_date,
            @RequestParam(required = false, defaultValue = "usuario") String orderBy,
            @RequestParam(required = false, defaultValue = "10") int pageSize,
            @RequestParam(required = false, defaultValue = "0") int pageNumber
            ) {

        Page<PersonOutputDto> personas = personService.buscarPersonas(usuario, name, surname, created_date, termination_date, pageSize, pageNumber);
